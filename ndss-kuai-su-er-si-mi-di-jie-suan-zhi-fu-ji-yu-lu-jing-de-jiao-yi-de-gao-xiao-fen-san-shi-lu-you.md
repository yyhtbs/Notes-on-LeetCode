...
```text
算法 1 setRoutes
# Input: Graph G, landmarks l1, . . . , l|L|
for i = 1 to |L| do
    idi(li) = ()
    q = empty queue
    add li to q
    bi = true # first consider nodes with links in both directions
    while q is not empty do
        ...
        for all n: neighbors of node do
            ...
            if idi(n) is not set then
                if (w(node, n) > 0 and w(n, node) > 0) or !bi then
                    parenti  (n) = node
...
setRoutes：在算法1所述的初始化阶段，我们得出嵌入。遍历所有界标，算法1将界标坐标分配为空矢量（第3行），并将界标附加到队列（第5行）。然后，算法的主循环处理队列。在每个步骤中，算法都会从队列中删除一个节点（第8行），并考虑其所有邻居。如果邻居n还没有坐标并且有资格拥有一个坐标，则该算法会将n集成到生成树中，通过将父坐标和一个随机的b位数字连接来分配坐标，并将其附加到队列中（ 14-17）。确定是否属于扩展范围的标准取决于算法的阶段：最初（$$b_i = true$$，第6行），如果到其潜在父节点的链路上的可用资金额度不为零，则该节点是合格的。在算法的第二阶段$$(b_i = false)$$，由空队列触发（第18-21行），所有节点都可以加入生成树。请注意，算法1不会阻止孩子选择父母，以使他们在任一方向上都没有资金。由于此类链路在PBT网络中没有任何作用，因此我们假定该网络不允许它们。可替代地，算法1可以在允许父子关系之前检查单向链路上的资金是否为非零。一旦队列为空，该算法即终止，表明所连接图中的所有节点都具有坐标。
